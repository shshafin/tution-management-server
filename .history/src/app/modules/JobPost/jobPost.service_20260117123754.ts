...
const createJobPostIntoDB = async (payload: TJobPost) => {
  ...
  if (config?.isOtpSecurityEnabled) {
    ...
    // এসএমএস পাঠানো (Utility ফাংশন কল করা)
  ...
  // ৪. যদি Admin OTP Toggle OFF থাকে, তবে সরাসরি Published করে দেওয়া
...
const getAllJobPostsFromDB = async () => {
  // শুধুমাত্র পাবলিশ হওয়া জবগুলো পাবলিকলি দেখাবে  const result = await JobPost.find({ status: 'published' }).sort({
    createdAt: -1,
  });
  return result;
};
