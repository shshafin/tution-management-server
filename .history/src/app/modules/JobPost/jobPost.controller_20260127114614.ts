...
const getTutorJobFeed = catchAsync(async (req: Request, res: Response) => {
  const queryData = {
    ...req.query,
    // 🟢 নিশ্চিত কর ডাটাগুলো ঠিকমতো পাস হচ্ছে
    tutorGender: req.user?.gender || req.query.tutorGender,
    tutorDiscipline: req.user?.discipline || req.query.tutorDiscipline,
  };

  // 🔍 ডিবাগিং এর জন্য এই লগটা মাস্ট দিবি চেক করতে
  console.log('Applied Filters:', {
    gender: queryData.tutorGender,
    discipline: queryData.tutorDiscipline,
  });
  ...
  sendResponse(res, {
    ...
    success: true,
    message: 'জব ফিড সফলভাবে লোড হয়েছে।',
    data: result,
  });
});
