...
const getTutorJobFeedFromDB = async (query: Record<string, any>) => {
  ...
  if (query.latitude && query.longitude && query.tutoringType === 'offline' && !searchTerm) {
    const config = await AdminConfig.findOne();
  ...
  let jobQuery = JobPost.find(finalQuery).select('-guardianPhone -location.detailedAddress');
