...
const createJobPostIntoDB = async (payload: TJobPost) => {
  ...
  if (config?.isOtpSecurityEnabled) {
    ...
    return {
      message: 'Job post created! Please verify OTP.',
      jobId: result._id,
      isOtpRequired: true,
    };
  }

  // ৩. ওটিপি অফ থাকলে সরাসরি পাবলিশ
...
// টিউটরদের জন্য ফিড (Contact Info Hidden)
const getTutorJobFeedFromDB = async (query: Record<string, unknown>) => {
  ...
  return await searchQuery
    .find(filterQuery)
    .select('-guardianPhone -guardianEmail') // ফোন-ইমেইল হাইড
    .sort('-createdAt');
};

// সিঙ্গেল জব ভিউ (Role-based Reveal Logic)
const getSingleJobPostFromDB = async (id: string, role?: string) => {
  const query = JobPost.findById(id);

  // যদি ইউজার টিউটর হয়, তবে কন্টাক্ট ডিটেইলস হাইড থাকবে
  if (role === 'tutor') {
