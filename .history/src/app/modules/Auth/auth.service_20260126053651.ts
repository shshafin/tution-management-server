...
const changePassword = async (userData: JwtPayload, payload: any) => {
  ...
  // ১. ইউজারকে খুঁজে বের করা (Password সহ select করতে হবে)
  const user = await User.findById(userData.userId).select('+password');
  if (!user) {
    throw new AppError(httpStatus.NOT_FOUND, 'User not found!', '');
  }

  // ২. পুরানো পাসওয়ার্ড ঠিক আছে কি না চেক করা
  ...
  // ৩. নতুন পাসওয়ার্ড সেট করা (মডেলের pre-save হুক পাসওয়ার্ড হ্যাশ করে দেবে)
  user.password = newPassword;
  await user.save();
