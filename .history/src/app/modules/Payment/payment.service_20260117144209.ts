...
const verifyAndConfirmPayment = async (invoiceId: string) => {
  // ১. ZiniPay থেকে পেমেন্ট ভেরিফাই করা
  const paymentData = await verifyPayment(invoiceId);

  if (!paymentData || paymentData.status !== 'COMPLETED') {
    throw new AppError(httpStatus.BAD_REQUEST, 'Payment verification failed or not completed!', '');
  }

  // ২. আমাদের ডাটাবেসে পেমেন্ট রেকর্ড খুঁজে বের করা
  const paymentRecord = await Payment.findOne({ invoiceId }).populate('package');

  if (!paymentRecord) {
    throw new AppError(httpStatus.NOT_FOUND, 'Payment record not found in database!', '');
  }
  ...
  // ৩. ইউজারের ক্রেডিট আপডেট করা (Transaction session ইউজ করা ভালো)
