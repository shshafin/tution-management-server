...
const initPayment = async (tutorId: string, packageId: string) => {
  ...
  if (!tutor || !pkg) throw new AppError(httpStatus.NOT_FOUND, 'Tutor/Package not found!', '');
  ...
  throw new AppError(httpStatus.BAD_REQUEST, 'Payment URL generation failed!', '');
};
...
const verifyAndConfirmPayment = async (gatewayInvoiceId: string) => {
  ...
  if (!paymentRecord) throw new AppError(httpStatus.NOT_FOUND, 'Payment record not found!', '');

  const pkg = paymentRecord.package as any;

  // ৪. ক্রেডিট আপডেট ও স্ট্যাটাস চেঞ্জ
  await User.findByIdAndUpdate(paymentRecord.tutor, { $inc: { credits: pkg.credits } });
