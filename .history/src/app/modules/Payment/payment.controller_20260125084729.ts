...
const handleWebhook = catchAsync(async (req: Request, res: Response) => {
  const payload = req.body; // ZiniPay এখানে পেমেন্ট ডাটা পাঠাবে

  // ZiniPay সাধারণত invoiceId এবং status পাঠায়
  if (payload.status === 'COMPLETED') {
    await PaymentService.verifyAndConfirmPayment(payload.invoiceId);
  }

  res.status(200).send('OK'); // ZiniPay-কে কনফার্ম করা যে ডাটা পেয়েছি
});
