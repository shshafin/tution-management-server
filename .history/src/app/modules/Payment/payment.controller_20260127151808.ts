...
const handleSuccess = catchAsync(async (req: Request, res: Response) => {
  ...
  try {
    // ১. পেমেন্ট ভেরিফাই করা এবং ডাটাবেজ আপডেট (ক্রেডিট অ্যাড হবে এখানে)
    await PaymentService.verifyAndConfirmPayment(invoiceId as string);

    // ২. সব ঠিক থাকলে ফ্রন্টএন্ডের সাকসেস পেজে পাঠিয়ে দেওয়া
    // সাথে কুয়েরি প্যারামিটারে ইনভয়েস আইডি দিচ্ছি যাতে সাকসেস পেজে ওটা দেখাতে পারিস
    res.redirect(
      `${config.frontend_payment_success_url}?invoiceId=${invoiceId}`,
    );
  } catch (error: any) {
    console.error('Payment Verification Error:', error);
    // ৩. কোনো সমস্যা হলে ফ্রন্টএন্ডের ওয়ালেট/ফেইল পেজে পাঠিয়ে দেওয়া
    res.redirect(
