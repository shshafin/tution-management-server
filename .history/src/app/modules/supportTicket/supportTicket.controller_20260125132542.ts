...
const getAllTickets = catchAsync(async (req: Request, res: Response) => {
  const result = await SupportTicketService.getAllTicketsFromDB(req.query);

  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
    message: 'All support tickets retrieved successfully',
    meta: result.meta, // 🛠️ Meta data (page, limit, total, totalPage)
    data: result.result, // 🛠️ Actual ticket data
  });
});

const getSingleTicket = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await SupportTicketService.getSingleTicketFromDB(id);
